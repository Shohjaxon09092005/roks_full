...
function ServiceFeedCard() {
    ...
    async function getEd() {
        let fetchEd = await fetch(`${URL}/service-feedback`);
        let jsonEd = await fetchEd.json();
        let sortedEd = jsonEd?.service_feedback.sort((a, b) => b.id - a.id);
        setEd(sortedEd)
    ...
    async function updateValue(e) {
        ...
        if (timeDiff < 9.5) {
            try {
                const responce = await fetch(`${URL}/service-feedback/${valueId}`, {
                    method: "PUT",
                    headers: {
    ...
    async function DeleteId(e, id) {
        ...
        if (timeDiff < 9.5) {
            try {
                const responceDel = await fetch(`${URL}/service-feedback/${id}`, {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
...
export default ServiceFeedCard
