...
function NewsFeedCard() {
    ...
    async function getEd() {
        let fetchEd = await fetch(`${URL}/news-feedback`);
        let jsonEd = await fetchEd.json();
        let sortedEd = jsonEd?.news_feedback.sort((a, b) => b.id - a.id);
        setEd(sortedEd)
    ...
    async function updateValue(e) {
        ...
        if (timeDiff < 9.5) {
            try {
                const responce = await fetch(`${URL}/news-feedback/${valueId}`, {
                    method: "PUT",
                    headers: {
    ...
    async function DeleteId(e, id) {
        ...
        if (timeDiff < 9.5) {
            try {
                const responceDel = await fetch(`${URL}/news-feedback/${id}`, {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
