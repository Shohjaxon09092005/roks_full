...
function DoctorFeedCard() {
    ...
    async function getEd() {
        let fetchEd = await fetch(`${URL}/doctor-feedback`);
        let jsonEd = await fetchEd.json();
        let sortedEd = jsonEd?.doctor_feedback.sort((a, b) => b.id - a.id);
        setEd(sortedEd)
    ...
    async function updateValue(e) {
        ...
        let formData = {
            doctor_id: filteredValue?.doctor_id,
        ...
        if (timeDiff < 9.5) {
            try {
                const responce = await fetch(`${URL}/doctor-feedback/${valueId}`, {
                    method: "PUT",
                    headers: {
    ...
    async function DeleteId(e, id) {
        ...
        if (timeDiff < 9.5) {
            try {
                const responceDel = await fetch(`${URL}/doctor-feedback/${id}`, {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
            ...
            <div ref={modalValue} className="modal_overlay_value">
                <div className="modal_value">
                    ...
                    <form onSubmit={(e) => updateValue(e)} >
                        <label htmlFor="ratingDoc">Reyting</label>
                        <input id='ratingDoc' type="text" value={filteredValue?.rating}/>
                        <label htmlFor="newRatingDoc">Yangi reyting</label>
                        <input id="newRatingDoc" type="text" ref={upd_rating}  onChange={handleChange}  value={value} />
                        <label htmlFor="mess">Xabar</label>
                        <textarea id='mess' ref={upd_message} defaultValue={filteredValue?.feedback} />
